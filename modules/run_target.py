...
def run_target():
    print 'CLONING TARGET REPOSITORY: PROCESS START'
    ...
    if not os.path.isdir(project_folder + '/' + git_project_name):
        if git_user is None or git_password is None:
            os.system(git_clone_cmd + git_target_url + ' ' + project_folder + '/' + git_project_name)
        else:
    ...
    os.system(python_exec_cmd + ' ' + project_folder + '/' + git_project_name + '/target_service/target.py &')
    print 'CLONING TARGET REPOSITORY: PROCESS DONE'
